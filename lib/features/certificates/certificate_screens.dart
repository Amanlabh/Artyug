...
class _CertificateListScreenState extends State<CertificateListScreen> {
  ...
  Future<void> _loadCerts() async {
    if (AppConfig.isDemoMode) {
      ...
      if (mounted)
        setState(() {
          _certs = CertificateModel.demoCerts;
          _loading = false;
        });
      return;
    }
    try {
      ...
      if (mounted)
        setState(() {
          _certs =
              (data as List).map((m) => CertificateModel.fromMap(m)).toList();
          _loading = false;
        });
    } catch (e) {
      if (mounted)
...
class _CertificateSquareTile extends StatelessWidget {
  ...
  static const _qrEye =
      QrEyeStyle(color: _onLight, eyeShape: QrEyeShape.square);
  static const _qrData = QrDataModuleStyle(
      color: _onLight, dataModuleShape: QrDataModuleShape.square);
          ...
          child: Container(
            ...
            child: QrImageView(
              ...
              eyeStyle: const QrEyeStyle(
                  color: _onLight, eyeShape: QrEyeShape.square),
...
class _EmptyView extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
        ...
        children: [
          ...
          const Text('Purchase artwork to receive authenticity certificates.',
              style: TextStyle(
                fontSize: 14,
          ...
          ElevatedButton(
            ...
            child: const Text('Browse Artworks',
...
class CertificateLoadingScreen extends StatefulWidget {
  ...
  @override
  State<CertificateLoadingScreen> createState() =>
...
class CertificateDetailScreen extends StatelessWidget {
  ...
  void _share(BuildContext context) {
    ...
    Clipboard.setData(ClipboardData(
      text:
          'Check out this authenticated artwork certificate: artyug.art/certificate/${cert.id}',
    ));
  }
}
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(children: [
          ...
          QrImageView(
            ...
            eyeStyle:
                const QrEyeStyle(color: _onLight, eyeShape: QrEyeShape.square),
            dataModuleStyle: const QrDataModuleStyle(
                color: _onLight, dataModuleShape: QrDataModuleShape.square),
          ),
          const SizedBox(height: 16),
          Text(
            ...
            style: const TextStyle(
                fontSize: 11, color: _onLightMuted, fontFamily: 'monospace'),
            textAlign: TextAlign.center,
          ),
          const SizedBox(height: 16),
...
class _BlockchainSection extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final chainMode = AppConfig.isDemoMode
        ? 'DEVNET'
        : AppConfig.chainMode.name.toUpperCase();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (!solanaEnabled) ...[
            _BlockedChip(
                solanaBlockReason ?? 'Blockchain anchoring is disabled'),
            ...
            Row(children: [
              const Text('TX Hash',
                  style: TextStyle(fontSize: 12, color: _onLightMuted)),
              const Spacer(),
              Flexible(
                  child: Text(cert.txHash!,
                      style: const TextStyle(
                        fontSize: 11,
                        fontWeight: FontWeight.w700,
                        color: AppColors.primary,
...
class _BlockchainRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
        ...
        children: [
          Text(label,
              style: const TextStyle(fontSize: 12, color: _onLightMuted)),
          Text(value,
              style: const TextStyle(
                  fontSize: 12, fontWeight: FontWeight.w700, color: _onLight)),
        ],
      );
}
        ...
        children: [
          Column(children: [
            Container(
              ...
              decoration: BoxDecoration(
                  color:
                      isFirst ? AppColors.primaryLight : AppColors.background,
                  shape: BoxShape.circle),
              child: Icon(icon,
                  size: 16,
            ...
            if (!isFirst)
              Container(width: 2, height: 0, color: AppColors.border),
          ]),
            ...
            child: Column(
              ...
              children: [
                ...
                Text(actor,
                    style: const TextStyle(fontSize: 12, color: _onLightMuted)),
                Text(date,
                    style: const TextStyle(fontSize: 11, color: _onLightHint)),
              ],
            ),
          )),
        ],
